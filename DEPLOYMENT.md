...
## תיקון בעיית Persistence - 27/01/2025

### הבעיה שזוהתה:
1. **משתמשים אחרונים לא מוצגים**: הכפתור "משתמשים אחרונים" הציג רק את המשתמש הנוכחי
2. **מגבלה חודשית מתאפסת**: המגבלה החודשית של 200 בדיקות מתאפסת בכל דיפלוי

### הסיבה:
המערכת הייתה מוגדרת להשתמש ב-SQLite (`USE_MONGODB=false`) שמשתמש בקובץ מקומי (`/var/data/watchbot.db`) שנמחק בכל דיפלוי חדש.

### הפתרון שיושם:

#### 1. מעבר ל-MongoDB
- שונה `USE_MONGODB` מ-`false` ל-`true` בקובץ `render.yaml`
- MongoDB מספק התמדה של נתונים בין דיפלויים

#### 2. הבטחת רישום משתמשים
הוספת קריאות ל-`db.add_user()` בכל הפונקציות החשובות:
- `watch_command` - הוספת נושא למעקב
- `list_command` - הצגת רשימת נושאים
- `whoami_command` - מידע על המשתמש
- `button_callback` - טיפול בכפתורים

#### 3. איחוד פונקציית סטטיסטיקות
- נוספה פונקציה `get_stats()` לשתי מחלקות הDB (SQLite ו-MongoDB)
- עדכון `stats_command` להשתמש בפונקציה המאוחדת

### קבצים שעודכנו:
1. `render.yaml` - שינוי `USE_MONGODB` ל-`true`
2. `main.py` - הוספת רישום משתמשים ופונקציות סטטיסטיקות
3. `VERSION` עודכן ל-`2025-01-27-15:00`

### תוצאות צפויות לאחר הדיפלוי:
✅ כפתור "משתמשים אחרונים" יציג את כל המשתמשים שהשתמשו במערכת  
✅ המגבלה החודשית תישמר בין דיפלויים  
✅ כל נתוני המשתמשים יישמרו באופן קבוע
